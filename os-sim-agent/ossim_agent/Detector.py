...
class Detector(threading.Thread):
    ...
    def send_message(self, event):
        ...
        #
        if self._conf.has_section("events-default"):

        # 1) date
            default_date_format = self._conf.get("events-default",
                                                 "date_format")
        ...
        # 2) sensor
            default_sensor = self._conf.get("events-default", "sensor")
            if event["sensor"] is None and default_sensor and \
               'sensor' in event.EVENT_ATTRS:
                event["sensor"] = default_sensor

        # 3) interface
            default_iface = self._conf.get("events-default", "interface")
