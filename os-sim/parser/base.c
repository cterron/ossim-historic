...
int
update_level (MYSQL * mysql, char *ip, char *type, int level)
{
...
int
update_nets_level (MYSQL * mysql, char *ip, char type, int level)
{
  ...
  query = (char *) malloc (sizeof (char) * QUERY_MAX_SIZE);
  snprintf (query, QUERY_MAX_SIZE,
	    "SELECT net_name FROM net_host_reference WHERE host_ip = '%s';",
	    ip);
  ...
  if (mysql_query (mysql, query))
      ...
      if ((result = mysql_store_result (mysql)))
	{
	  while ((row = mysql_fetch_row (result)))
	    {
	      if (!update_net_level (nets, row[0], type, level))
		return 0;
	    }
	  success = 1;
...
int
insert_level (MYSQL * mysql, char *ip, int levelC, int levelA)
{
  ...
  if (mysql_query (mysql, query))
    {
// Dirty hack, we don't mind if query fails
      ;
    }
  else
    {
...
int
get_sensor_id (MYSQL * mysql, char *sensor_fw, unsigned long int *sensor_id)
{
  MYSQL_RES *result;
  MYSQL_ROW row;
...
int
get_sig_id (MYSQL * mysql, char *action, unsigned long int *sig_id)
{
  MYSQL_RES *result;
  MYSQL_ROW row;
  unsigned int num_rows;
...
int
get_sig_class_id (MYSQL * mysql, unsigned long int *sig_class_id)
{
  MYSQL_RES *result;
  MYSQL_ROW row;
  unsigned int num_rows;
...
static int
get_last_cid (MYSQL * mysql, unsigned int sid, unsigned long int *cid)
{
  MYSQL_RES *result;
  MYSQL_ROW row;
  unsigned int num_rows;
...
int
insert_event (MYSQL * mysql, unsigned long int sid, unsigned long int sig_id,
	      unsigned long int sig_class_id, char *new_sig_name,
	      unsigned long int src_ip, unsigned long int dst_ip,
	      unsigned int src_port, unsigned int dst_port, char *protocol)
{
  char *query_snort;
  char *query_acid;
  char *query_sensor;
  ...
  snprintf (query_acid, QUERY_MAX_SIZE, "INSERT INTO acid_event VALUES\
             (%lu, %lu, %lu, '%s', %lu, 2, now(), %lu, %lu,\
              %u, %u, %u);", sid, cid + 1, sig_id, new_sig_name, sig_class_id, src_ip, dst_ip, proto, src_port, dst_port);
