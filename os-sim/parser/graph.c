...
static int
get_graph_id (MYSQL * mysql, char *ip, int *graph_id)
{
...
static void
link (MYSQL * mysql, char *source_ip, char *dest_ip)
{
	  ...
	  (num_rows = mysql_num_rows (result)))
	{
	  snprintf (query, QUERY_MAX_SIZE,
		    "UPDATE link SET occurrences = occurrences + 1\
                      WHERE source = '%s' AND dest = '%s';", source_ip, dest_ip);
	  if (mysql_query (mysql, query))
	    {
	      fprintf (stderr, "Failed to make query %s\n%s\n",
		       query, mysql_error (mysql));
...
void
graph (MYSQL * mysql, char *source_ip, char *dest_ip)
{
