        ...
        if t==EventSnort.TYPEALARM:
            ...
            while 1:
                ...
                #Further options should be added here
                    ...
                    logger.debug("Unsupported Ethertype "+str(self._ethertype))
...
class ParserSnort(object):
    ...
    def _update_log_list(self):
        ...
        for f in dircontents:
            ...
            if not hasattr(self, "_timestamp"): #log directory is empty (init_log_dir->len(dircontents) could be also tested)
                self._timestamp = 0
        ...
        while 1:
            ...
            if self._fd == None:
                if len(self._logdir)==0:
                    ...
                    logger.debug("self._timestamp: %s" % self._currentfile[self._currentfile.rindex('.')+1:])
                    try:
                        self._fd =  open(self._currentfile,"r")
                    except IOError:
                        logger.error("Error reading file %s : it doesn't exists" % self._currentfile)
                    self._hdrread = False
            # Read the header
