...
function load_search_handlers()
{
    ...
    $('[data-bind="import-siem"]').on('click', function()
    {
        __asset_list.import_siem()
    });
    ...
    $('#delete_selection').on('click', function()
    {
        ...
        var msg = "<?php echo Util::js_entities(_('Are you sure you want to permanently delete ### assets?'))?>".replace('###', n_assets);
    ...
    $('#edit_selection').on('click', function()
    {
        if (!__asset_list.action_enabled(this))
        {
             return false;
    ...
    $('[data-bind="enable-monitoring"]').on('click', function()
    {
        __asset_list.toggle_monitoring('enable')
    });
    
    $('[data-bind="disable-monitoring"]').on('click', function()
    {
        __asset_list.toggle_monitoring('disable')
    });
    
    $('[data-bind="save-to-group"]').on('click', function()
    {
        __asset_list.save_to_group()
    });
...
function GB_onhide(url, params)
{
    ...
    if (cond_group)
    {
        if (id != '')
        {
            __asset_list.load_group_detail(id);
        }
    }
    else if (cond_force)
    {
    ...
    {
        __asset_list.reload_table();
    }
    else if (cond_notes)
    {
        var n_assets = __asset_list.get_num_selected_assets();
        var msg = "<?php echo Util::js_entities(_('Your note has been added to ### assets.')) ?> ".replace('###', n_assets);
...
function av_asset_list(perms)
{
    ...
    this.draw_asset_list = function()
    {
        ...
        __self.datatables_assets = $('.table_data').dataTable(
        {
            ...
            "bServerSide": true,
            "bDeferRender": true,
            "sAjaxSource": __self.cfg.asset.providers + "load_assets_result.php",
            "iDisplayLength": 20,
            "bLengthChange": true,
            ...
            "aoColumns": [
                ...
                { "bSortable": true,  "sClass": "left", "sWidth": "180px"},
                { "bSortable": true,  "sClass": "left", "sWidth": "180px"},
    ...
    this.delete_selection = function()
    {
        ...
        __self.save_selection().done(function()
        {
            ...
            var h_data = 
            {
                "token" : Token.get_token("delete_asset_bulk")
            };
        
            $.ajax(
            {
                type: "POST",
                url: __self.cfg.asset.controllers  + "bk_delete.php",
                data: h_data,
                dataType: "json",
                beforeSend: function()
                {
    ...
    this.edit_asset = function()
    {
        ...
        __self.save_selection().done(function()
        {
            ...
            var title = "<?php echo Util::js_entities(_('Edit Asset')) ?>";
