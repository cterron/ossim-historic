...
if ($action == "generate") {
    ...
    $hosts = "";
    for ($i = 0; $i < $nhosts; $i++) {
        if ($hosts == "") $hosts = REQUEST("host$i");
        else if (REQUEST("host$i") != "") $hosts.= "," . REQUEST("host$i");
    }
...
<h3><?php echo _("Select hosts for custom report") ?></h3>
<?php echo _("Note: Only hosts that have been scanned are available for selection") ?>.

<form action="<?php echo $_SERVER["SCRIPT_NAME"] ?>" method="POST">
<?php echo _("Report title"); ?> <input type="text" name="title" size="50">
...
?>
                <div align="left">
                <input align="left" type="checkbox" id="<?php echo "host" . $global_i ?>" name="<?php echo "host" . $global_i ?>" value="<?php echo $host->get_ip() ?>" /><?php echo Host::ip2hostname($conn, $host->get_ip()); ?></div></td>
                <?php
            $global_i++;
        }
        echo "</tr>\n";
...
<input type="hidden" name="nhosts" value="<?php
    echo $global_i ?>" />
