...
{         
        ...
        'hosts' => array(
            ...
            'help' => array(                
                'Version 4.x.x' => array(
                    ...
                    'example' => '"192.168.10.3";"Host_1";"www.example-1.es,www.example-2.es";"'._('Short description').'";"2";"Windows";"23.78";"121.45";"379D45C0BBF22B4458BD2F8EE09ECCC2";0;"Server:Mail Server"'
                ),
                'Version 3.x.x' => array(
                    ...
                    'example' => '"192.168.10.3";"Host_1";"www.example-1.es,www.example-2.es";"'._('Short description').'";"2";"192.168.10.2,192.168.10.3";"Windows";"23.78";"121.45"'
                )               
            )         
        ),
        'welcome_wizard_hosts' => array(
            ...
            'help' => array(               
                'Version 4.x.x' => array(
                    ...
                    'example' => '"192.168.10.3";"Host_1";"'._('Short description').'";"Windows";"Server,Network Device"'
                )            
            )         
        )
    );
	...
	else
	{
    	...
    	<form name='form_csv' id='form_csv' method='POST' action='import_all_hosts_ajax.php' enctype='multipart/form-data' target='iframe_upload'>        
            ...
            <table id='form_container'>
    			...
    			{ 
    				...
    				<tr class='left'>
    				    <td class="td_title">
						    <span><?php echo _('Select the Entity for the hosts');?>:</span>
						</td>
    				</tr>        
...
function import_assets_from_csv($filename, $iic, $ctx, $import_type)
{
    ...
    /****************************************************************************************************************************
     * From host section:
     *  - Version 4.x.x: "IP*(IP1,IP2,...)";"Hostname";"FQDNs(FQDN1,FQDN2,...)";"Description";"Asset value";"Operating System";
     ...
     *
     * From welcome wizard:
     *  - Version 4.x.x: "IP*(IP1,IP2,...)";"Hostname";"Description";"Operating System";"Device Type(Type1,Type2,...)"   
     *
    *****************************************************************************************************************************/
	...
	{						
		...
		// Special case: Forced Host ID [Version 4.x.x]
        ...
        if (count($summary['by_hosts'][$num_line]['errors']) == 0)
        {
            try
            {
                ...
                {                                        
    				...
    				{                    			
                		...
                		else
                		{
                    		...
                    		if ($cnd_1 && !$cnd_2) 
                    		{
                                $c_error_msg = _('IP').' '.$csv_ips.' '._("not allowed. IP $ip out of range. Check your asset filter");
                ...
                {
                    $c_error_msg = _('Host').' '.$name.': '._("IP not allowed. IP wasn't be modified"); 
                ...
                if (count($summary['by_hosts'][$num_line]['errors']) == 0)
                {
                    ...
                    $summary['by_hosts'][$num_line]['data'] = ($is_in_db == TRUE) ? _('Host updated') : _('New host inserted'); 
	...
	if ($summary['general']['statistics']['saved'] > 0)
	{
    	if ($summary['general']['statistics']['errors'] == 0)
    	{
        	...
        	$summary['general']['data']   = _('All hosts have been imported successfully');
    	}
    	else
    	{
        	$summary['general']['status'] = 'warning';
        	$summary['general']['data']   = _('Some hosts could not be imported successfully');        	
    	}      	
	...
	{	       
    	...
    	{
        	$summary['general']['status'] = 'error';
        	$summary['general']['data']   = _('Hosts could not be imported');
    	}
	}
