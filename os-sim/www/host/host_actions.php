...
switch($action)
{
    ...
    case 'toggle_a_monitoring':
        ...
        if ($data['status'] == 'error')
        {
            ...
            //Formatted message
            $data['data'] = '<div>'._('We Found the following errors').":</div><div style='padding: 5px;'>".$e_message.'</div>';
        }
    ...
    case 'delete_properties':
        ...
        //Enable o disable Nagios in host only if we have deleted a service.
        if ($reload_nagios)
        {
        ...
        try
        {
            ...
            switch($p_id)
            {
                ...
                case '40':
                    ...
                    $protocol_id = Protocol::get_protocol_by_name(POST('protocol'));
            ...
            else
            {
                ...
                //Enable Nagios in host only if we have enabled Nagios for it.
                if ($reload_nagios)
                {
                    Asset_host_scan::save_plugin_in_db($conn, $p_data['host_id'], 2007);
                }
