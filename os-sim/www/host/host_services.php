                    ...
                    $("#check_all").on('click', function(e) {
                        ...
                        if ($(layer).length >= 1)
                        {
                            $(layer).dynatree("getRoot").visit(function(node){
                                ...
                                //Only service nodes
                                if (node.data.key.match(/^values_by_property_branch_/))
                                {
                                    node.select(true);
                                }
                            });
                ...
                function load_tree()
                {
                    ...
                    $(layer).dynatree({
                        ...
                        onPostInit: function(isReloading, isError) {
                            ...
                            var tree = $(layer).dynatree("getTree");
                            var node = tree.getNodeByKey("no_services_found");

                            //Node 'No services found' doesn't exists
                            if (typeof(node) == 'undefined' || node == null)
                            {
                                $('#check_all').removeClass('av_l_disabled');
                                $('#uncheck_all').removeClass('av_l_disabled');
                            }
                        },
                ...
                ****************************************************/
                ...
                if ($is_ext_ctx == FALSE)
                {
                    ...
                    ***************************************************/
                    ...
                    //Local scan
                    $('#scan_host').click(function() {

                        top.frames['main'].go_to_asset_discovery();
                    });
        ...
        <div id="host_container">
            ...
            <table id="t_container">
                ...
                else
                {
                    ...
                    {
                        $t_scan  = _('Discovering host properties using NMAP scan');
                        $s_title = _('Scan host now');

                        $title .= ' [ <a class="info" id ="scan_host" title="'.$t_scan.'">'.$s_title.'</a> ]';
                    }
                    ?>
                        ...
                        <tr>
                            <td>
                                <table id="t_actions">
                                    <tr>
                                        <td class='left'>
                                            <a href="javascript:void(0);" class='av_l_disabled' id='check_all'><?php echo _('Check All')?></a>
                                            &nbsp;|&nbsp;
                                            <a href="javascript:void(0);" class='av_l_disabled' id='uncheck_all'><?php echo _('Uncheck All')?></a>
                                        </td>
                                        <td class='right'>
