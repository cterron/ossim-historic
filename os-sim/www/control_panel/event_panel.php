...
//function comillas ($txt) { return str_replace("'","\'",$txt) }
// responder js
if (GET('modo') == "responder") {
    ...
    if ($from_snort) {
        ...
        $where = ($plugins != "") ? "where sid in ($plugins) AND timestamp>CURDATE()" : "";
        $index = ($plugins != "") ? "sid" : "timestamp";
        //$sql = 'select (select plugin_id from ossim_event where ossim_event.sid = acid_event.sid and ossim_event.cid = acid_event.cid '.$where.') as plugin_id, (select plugin_sid from ossim_event where ossim_event.sid = acid_event.sid and ossim_event.cid = acid_event.cid '.$where.') as plugin_sid, unix_timestamp(timestamp) as id, sig_name as plugin_sid_name, inet_ntoa(ip_src) as aux_src_ip, inet_ntoa(ip_dst) as aux_dst_ip, timestamp, ossim_risk_a as risk_a, ossim_risk_c as risk_c, (select substring_index(substring_index(hostname,":",1),"-",1) from sensor where sensor.sid = acid_event.sid) as sensor, layer4_sport as src_port, layer4_dport as dst_port, ossim_priority as priority, ossim_reliability as reliability, ossim_asset_src as asset_src, ossim_asset_dst as asset_dst, ip_proto as protocol, (select interface from sensor where sensor.sid = acid_event.sid) as interface from acid_event ORDER BY timestamp DESC limit '.$max_rows;
        $sql = 'select "0" as plugin_id,"0" as plugin_sid, unix_timestamp(timestamp) as id, sid, sig_name as plugin_sid_name, inet_ntoa(ip_src) as aux_src_ip, inet_ntoa(ip_dst) as aux_dst_ip, timestamp, ossim_risk_a as risk_a, ossim_risk_c as risk_c, (select substring_index(substring_index(hostname,":",1),"-",1) from sensor where sensor.sid = acid_event.sid) as sensor, layer4_sport as src_port, layer4_dport as dst_port, ossim_priority as priority, ossim_reliability as reliability, ossim_asset_src as asset_src, ossim_asset_dst as asset_dst, ip_proto as protocol, (select interface from sensor where sensor.sid = acid_event.sid) as interface from acid_event force index(' . $index . ') ' . $where . ' order by timestamp desc limit ' . $max_rows;
...
<table border=0 cellpadding=0 cellspacing=0 class="nobborder"><tr><td class="nobborder">
	...
	-->
	<SELECT NAME="speed" Onchange="reload()" disabled>  
...
<br>
<div id="cab" style="position:absolute;left:30px;top:90px">
<table border=0 cellpadding=0 cellspacing=0 class="nobborder" width="900" align="center">
<tr height="22">
  <td width="120" style="text-align:left"><b><?php echo _("Date"); ?></b></td>
...
<div id="tr" style="position:absolute;left:30px;top:120px">
<?php
    for ($i = 0; $i < $max_rows; $i++) { ?>
...
?>" class="opaque">
	<table border=0 cellpadding=0 cellspacing=0 class="nobborder" width="900" align="center">
	<tr height="22">
	  <td width="120" style="color:white;text-align:left" id="trdate<?php echo $i
?>">&nbsp;</td>
...
<div id="str" style="position:absolute;left:30px;top:120px">
<table border=0 cellpadding=0 cellspacing=0 class="nobborder" width="900" align="center">
<?php
    for ($i = 0; $i < $max_rows; $i++) { ?>
<tr height="22">
  <td width="120" style="text-align:left" id="date<?php echo $i
?>"></td>
...
<div id="filters" style="position:absolute;left:940px;top:90px">
...
<?php
    if ($from_snort) {
        ...
        foreach($sids as $pl => $arr) {
            $val = implode(",", $arr);
            echo "<input type='checkbox' class='little' value='$val'>$pl<br>\n";
        }
    }
?>
</form>
