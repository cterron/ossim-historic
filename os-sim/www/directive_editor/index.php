...
ossim_valid($_GET["enable"], OSS_LETTER, OSS_DIGIT, OSS_SCORE, OSS_NULLABLE, 'illegal:' . _("enable"));
ossim_valid($_GET["disable"], OSS_LETTER, OSS_DIGIT, OSS_SCORE, OSS_NULLABLE, 'illegal:' . _("disable"));
if (ossim_error()) {
    die(ossim_error());
}
...
<html>
	...
	<frame src="top.php?<?php echo $_SERVER['QUERY_STRING'] ?>">
		<frameset id="frames" cols="<?php echo $cols ?>" frameborder=1 framespacing=0>
			<?php
if ($_GET['directive'] != '') {
    $action = $_GET['action'];
    if ($action == 'add_rule') {
