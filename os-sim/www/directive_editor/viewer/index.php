...
<?php
if ($directive_id == '') $XML_FILE = '/etc/ossim/server/directives.xml';
else $XML_FILE = get_directive_file($directive_id);
...
if (!$dom = domxml_open_file($XML_FILE, DOMXML_LOAD_SUBSTITUTE_ENTITIES)) {
    echo "Error while parsing the document\nMake sure that $XML_FILE begins with xml tag!\n";
    exit;
}
$_SESSION['XML_FILE'] = $XML_FILE;
if (!empty($directive_id)) {
    if (is_free($directive_id) == "true") $direct = unserialize($_SESSION['directive']);
    else {
...
<tr><td><a href="../right.php?action=add_file&id=0" TARGET="right" TITLE="<?php
