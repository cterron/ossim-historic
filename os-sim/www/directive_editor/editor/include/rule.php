...
class Rule {
     ...
     * The constructor.
     */
    function Rule($id, $level, $rule, $name, $plugin_id, $plugin_type, $plugin_sid, $from, $port_from, $to, $port_to, $protocol, $sensor, $occurrence, $time_out, $reliability, $condition, $value, $interval, $absolute, $sticky, $sticky_different, $groups) {
        $this->id = $id;
        $this->level = $level;
        $this->rule = $rule;
        $this->name = $name;
        ...
        $this->sticky = ($sticky == "") ? "Default" : $sticky;
        $this->sticky_different = ($sticky_different == "") ? "Default" : $sticky_different;
        $this->groups = ($groups == "") ? "Default" : $groups;
    }
    ...
    <?php
		...
		<?php
            ...
            if (!$this->is_new()) {
                ...
                print '</td>';
                for ($i = 0; $i < 4; $i++) print '<td>&nbsp&nbsp&nbsp&nbsp&nbsp</td>';
            }
            if ($this->is_new()) { ?>        
...
<?php
    ...
    function getXMLNode($dom) {
        ...
        if ($this->type != NULL && $this->type != "") $node->set_attribute("type", $this->type);
        ...
        if ($this->sticky != "Default" && $this->sticky != "") $node->set_attribute("sticky", $this->sticky);
        if ($this->sticky_different != "Default" && $this->sticky_different != "") $node->set_attribute("sticky_different", $this->sticky_different);
        if ($this->groups != "Default" && $this->groups != "") $node->set_attribute("groups", $this->groups);
        if ($this->nb_child > 0) {
