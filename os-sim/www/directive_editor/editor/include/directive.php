...
?>
    ...
    <table align="center">
      <tr><th colspan=<?php
        ...
        print "$name<br>Directive $id (Priority: $priority ) "; ?></a></th></tr>
      <tr>
 		<?php
        for ($i = 0; $i < $level; $i++) print '<td>&nbsp&nbsp&nbsp&nbsp&nbsp</td>';
?>	
		<td>&nbsp&nbsp<a TARGET="right" href="../include/utils.php?query=add_rule&id=<?php
        echo $newid; ?>" TITLE="<?php
...
function getDirectiveFromXML($dom, $directive_id) {
    $_POST['ind'] = 0;
    $tab_rules = array();
    ...
    if ($directive->has_child_nodes()) {
        ...
        foreach($rules as $rule) {
            if (($rule->type == XML_ELEMENT_NODE) && ($rule->tagname() == 'rule')) {
                ...
                $temp = new Rule($id, $level, $rule, $rule->get_attribute('name') , $rule->get_attribute('plugin_id') , $rule->get_attribute('type') , $rule->get_attribute('plugin_sid') , $rule->get_attribute('from') , $rule->get_attribute('port_from') , $rule->get_attribute('to') , $rule->get_attribute('port_to') , $rule->get_attribute('protocol') , $rule->get_attribute('sensor') , $rule->get_attribute('occurrence') , $rule->get_attribute('time_out') , $rule->get_attribute('reliability') , $rule->get_attribute('condition') , $rule->get_attribute('value') , $rule->get_attribute('interval') , $rule->get_attribute('absolute') , $rule->get_attribute('sticky') , $rule->get_attribute('sticky_different') , $rule->get_attribute('groups'));
                $tab_rules[$ind] = $temp;
