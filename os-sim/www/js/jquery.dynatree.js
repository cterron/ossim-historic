/*************************************************************************
	...
	Modified by jmalbarracin 2009-06-02
	Added/Imported getAllNodes function from 0.4
...
DynaTree.version = "$Version: 0.5.0_rc-1$"; 
...
//		return $("#" + this.options.idPrefix + key).attr("dtnode");
	...
	getSelectedNodes: function(stopOnParents) {
        ...
        getAllNodes: function(skey) {
                var allnodeList = [];
                var nodeList = [];   
                this.tnRoot.visit(function(dtnode){
                        allnodeList.push(dtnode); 
                });
                for (var i=0;i<allnodeList.length;i++)
                        if (allnodeList[i].data.key.match(skey))
                                nodeList.push(allnodeList[i]);  
                return nodeList;
        },
