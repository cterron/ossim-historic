...
changeStatus funcion added
Drag&Drop for rows added handled by onToggleRow and onToggleGrid => special auto-create divs to toogle rows between different grids
Added viewTableToogle / onTableToggle trigger function
...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 idGroup: '',
			 nameGroup: '',
			 userdata1: '', // variable 1
			 userdata2: '', // variable 2
			 userdata3: '', // variable 3
		...
		var g = {
				...
				if (this.colresize)
					{
						...
						if (p.onColumnChange) p.onColumnChange(this.columnInfo(),g,p.laststat); // column info
					}
				else if (this.vresize)
					{
						this.vresize = false;
						//if (p.onEndResize) p.onEndResize(p.width,p.height); // column info
						if (p.onEndResize) p.onEndResize(this.columnInfo(),g,p.laststat); // column info
					}
				else if (this.colCopy)
					{
						$(this.colCopy).remove();
			...
			columnInfo: function(){ // get column info to save in a cookie or database
				var aData = [];
				var grid_height = $(g.bDiv).height();
				$('thead tr:first th',g.hDiv).each (
					function () {
						var gdiv = $('div', this);
						var colInfo = gdiv.css('width').toString(); // serialize format: width;sort;visible;index;name
						if (gdiv.hasClass('sasc'))
							colInfo = colInfo + ';asc';
						else if (gdiv.hasClass('sdesc'))
							colInfo = colInfo + ';desc';
						else
							colInfo = colInfo + ';none';
						colInfo = colInfo + ($(this).is(':visible') ? ';1' : ';0');
						colInfo = colInfo + ';' + $(this).attr('axis').substr(3);
						colInfo = colInfo + ';' + $(this).attr('cn'); //$(this).attr('abbr');
						colInfo = colInfo + ';' + grid_height;
						aData.push(colInfo);
					}
				);
				return aData;
			...
			populate: function () { //get latest data
				...
				{
					$(g.rowCopy).remove();
					if (g.drowp!=null) $(g.drowp).toggleClass('trSelectedCopy');
					g.drow = null;
					$("#srcrow").html('');
					$("#dstrow").html('');
					g.drowp = null;
								...
								function (e)
									{
										...
										if (!g.rowCopy && (p.onToggleRow || p.onToggleGrid))
										{
								...
								function (e)
									{
										...
										if (g.rowCopy)
										{
											if (e.pageX > g.hset.right || e.pageX < g.hset.left || e.pageY > g.hset.bottom || e.pageY < g.hset.top)
												...
												else $('body').css('cursor','url(../pixmaps/theme/grabbing.cur),auto');
											}
								...
								function (e)
									{
										...
										if (g.rowCopy)
										{
											$("#rowCopy").css({display: 'none'});
											if (g.drowp!=null) $(g.drowp).toggleClass('trSelectedCopy');
											if (g.drow != null && g.hrow != null && g.drow != g.hrow)
											{
												// external function handle rows switch
												if (p.onToggleRow) p.onToggleRow(g);
											}
											g.drow = null;
											$("#srcrow").html('');
											$("#dstrow").html('');
										...
										else if ($("#srcrow").html()!='' && $("#dstrow").html()!='') {
											$("#rowCopy").css({display: 'none'});
										...
										if (e.button == 2 && p.contextMenu) { // right buttom
											...
											$('#'+p.contextMenu).find('A').unbind('click');
											$('#'+p.contextMenu).find('LI:not(.disabled) A').click(function() {
												$(document).unbind('click');
												$(".contextMenu").hide();
												// Callback
												if (p.onContextMenuClick) p.onContextMenuClick($(this).attr('href').substr(1),rowid,g,p);
												return false;
											});
											$("#"+p.contextMenu).css({ top: e.pageY - 12, left: e.pageX -20 }).fadeIn(p.inSpeed);
											$("#"+p.contextMenu).add('UL.contextMenu').bind('contextmenu', function() { return false; }); // disabke browser context menu
										}
										else 
										$('body').css('cursor','default');
										$('body').noSelect(false);
			...
			for (i=0;i<p.colModel.length;i++)
				{
					...
					// always asign name for columninfo
					$(th).attr('cn',cm.name);
		...
		if (p.width!='auto' && p.width.toString().indexOf('%') < 0) g.gDiv.style.width = p.width + 'px';
					...
					if (!btn.separator)
					{
						...
						if (btn.onpress)
						{
							...
							(	
								function () 
								{
								this.onpress(this.name,g.gDiv,g,p);
								}
							);
						}
						$(tDiv2).append(btnDiv);
						...
						.hover(
							function(){
								...
								if (g.colCopy) 
									...
									$(g.nBtn).css({'left':nl,top:g.hDiv.offsetTop});
									if (hindex==0) $(g.nBtn).show(); // only for the first column
		...
		$(t).before(g.bDiv);
		$(g.bDiv)
		.css({ height: (p.height=='auto' || p.height.toString().indexOf('%') >= 0) ? 'auto' : p.height+"px"})
	...
	$.fn.flexigrid = function(p) {
		...
		//create special divs for toggle rows
		if ($("#rowCopy").html() == null) {
			var el = document.createElement("div");
			el.id = "rowCopy";
			el.className = "rowCopy";
			if ($.browser.msie) el.className = "rowCopy ie";
			el.innerHTML = "";
			$(el).css({position:'absolute',float:'left',display:'none', textAlign: 'center'});
			$('body').append(el);
		}
		if ($("#srcrow").html() == null) {
			var el = document.createElement("div");
	...
	$.fn.changeStatus = function(message,state) {
		return this.each(function() {
			if (this.grid) {
	...
	$.fn.viewTableToggle = function() {
		return this.each(function() {
			if (this.grid) {
				...
				$('div.ptogtitle',this.grid.mDiv).toggleClass('vsble');
				var state = ($(this.grid.gDiv).attr("class").match(/hideBody/)) ? 'close' : 'open';
				if (this.p.onTableToggle) this.p.onTableToggle(this.p,state);
			}
		});
	}; // end viewTableToggle
