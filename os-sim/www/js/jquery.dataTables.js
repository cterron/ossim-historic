...
(/** @lends <global> */function($, window, document, undefined) {
	...
	var DataTable = function( oInit )
	{
		...
		function _fnBuildHead( oSettings )
		{
			...
			if ( oSettings.bJUI )
			{
				for ( i=0, iLen=oSettings.aoColumns.length ; i<iLen ; i++ )
				{
					...
					{
					   nDiv.className += ' DataTables_no_sort';
					}
			...
			for ( var i=0 ; i<aDom.length ; i++ )
			{
				...
				if ( cOption == '<' )
				{
					...
					if ( cNext == "'" || cNext == '"' )
					{
						...
						if ( sAttr == "H" )
						{
							sAttr = "dt_header fg-toolbar ui-toolbar ui-widget-header ui-helper-clearfix";
						}
						else if ( sAttr == "F" )
						{
							sAttr = "dt_footer fg-toolbar ui-toolbar ui-widget-header ui-helper-clearfix";
						}
		...
		function _fnFeatureHtmlFilter ( oSettings )
		{
			...
			var pholder_check = document.createElement('input');
			
			if(sSearchStr_copy !== "" && !('placeholder' in pholder_check)) 
            {
                jqFilter.bind('focus', function () 
                {
