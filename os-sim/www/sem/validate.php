...
if ($logfile != "") {
    ...
    foreach($results as $result) {
        ...
        if (preg_match("/\s+(\d+)\s+(.*$)/", $res[0], $matches)) {
            $found_str = "Found in $result at line number " . $matches[1];
            error_log("$found_str\n", 3, "/tmp/validate");
...
if ($validate_file != "" && file_exists($validate_file)) {
    if (file_exists($validate_file . ".sig")) {
        ...
        print "Signature file not found. If the event is less than one hour old it will not be generated yet.";
        exit;
    }
} else {
    print "Logline not found in any logfiles";
    exit;
}
if ($verified == 1) {
    $verification_str = "Verification <font color=\"green\">OK</font><br/>";
} else if ($verified == 0) {
...
<center>
<table border="0">
<th colspan="2"><center> Log verification results </center></th>
<?php
print "<tr><td><b>Logline</b>:</td><td> $log_line</td></tr>";
print "<tr><td colspan=\"2\"><hr></tr></td>";
print "<tr><td colspan=\"2\" nowrap>$found_str</td></tr>";
print "<tr><td colspan=\"2\">$verification_str</td></tr>";
?>
</table>
</center>
</body>
