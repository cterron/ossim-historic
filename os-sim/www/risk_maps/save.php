...
ossim_valid($id, OSS_DIGIT, OSS_NULLABLE, 'illegal:'._("map"));
...
ossim_valid($name, OSS_NULLABLE, OSS_SCORE, OSS_ALPHA, OSS_DIGIT, OSS_SPACE, ";,.:\/\?=&()%&", 'illegal:'._("data"));
ossim_valid($icon, OSS_NULLABLE, OSS_SCORE, OSS_ALPHA, OSS_DIGIT, OSS_SPACE, ";,.:\/\?=&()%&", 'illegal:'._("data"));
...
die(ossim_error());
	...
	$params = array($map);
	  ...
	  if($icon != ""){
	  	if ($id !="" && $name !="") {
	      		$query = "update risk_indicators set icon= ?, name= ?, url= ? where id= ?";
			$params = array($icon, $name, $url, $id);
	      		$conn->Execute($query,$params);
	      		echo "refresh_indicators();\n";
		}
	  } else {
	  	if ($id !="" && $name !="") {
	      		$query = "update risk_indicators set  name= ?, url= ? where id= ?";
			$params = array($name, $url, $id);
	      		$conn->Execute($query,$params);
	      		echo "refresh_indicators();\n";
		}
	  }
