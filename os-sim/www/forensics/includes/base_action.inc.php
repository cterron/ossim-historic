        ...
        */
        if (($using_blobs)) {
            ...
            if ($context == PAGE_STAT_ALERTS) {
                ...
                $sql = "SELECT acid_event.sid, acid_event.cid " . $action_sql . " AND signature='" . $tmp . "'";
                $sql2 = "SELECT count(acid_event.sid) " . $action_sql . " AND signature='" . $tmp . "'";
            }
...
function Action_del_alert_post($action_arg, &$action_ctx, $db, &$num_alert, $action_cnt, $context, $deltmp) {
    ...
    if ($deltmp != "") {
        ...
        shell_exec("nohup cat $deltmp | /usr/bin/ossim-db snort > /dev/null 2> /dev/null &");
...
function PurgeAlert_ac($sid, $cid, $db) {
    ...
    if ($myrow = $res->baseFetchRow()) {
        ...
        $ip_src = $myrow['ip_src'];
        $ip_dst = $myrow['ip_dst'];
        $sig_name = str_replace("'", "\'", $myrow['sig_name']);
        $sig_class_id = $myrow['sig_class_id'];
        ...
        if ($signature != "" && $ip_src != "" && $ip_dst != "") {
            // AC_SENSOR
            $delsql.= "update ignore ac_sensor_sid set cid=cid-1 WHERE sid=$sid and day='$day';\n";
            $delsql.= "delete from ac_sensor_signature WHERE sid=$sid and day='$day' and signature=$signature;\n";
            ...
            // AC_ALERTS
            $delsql.= "update ignore ac_alerts_signature set sig_cnt=sig_cnt-1 WHERE signature=$signature and sig_name='$sig_name' and sig_class_id=$sig_class_id and day='$day';\n";
            $delsql.= "delete from ac_alerts_sid WHERE signature=$signature and day='$day' and sid=$sid;\n";
            $delsql.= "delete from ac_alerts_ipsrc WHERE signature=$signature and day='$day' and ip_src=$ip_src;\n";
            $delsql.= "delete from ac_alerts_ipdst WHERE signature=$signature and day='$day' and ip_dst=$ip_dst;\n";
            // AC_ALERTSCLAS
            $delsql.= "update ignore ac_alertsclas_classid set cid=cid-1 WHERE sig_class_id=$sig_class_id and day='$day';\n";
            ...
            // AC_SRCADDR
            $delsql.= "update ignore ac_srcaddr_ipsrc set cid=cid-1 WHERE ip_src=$ip_src and day='$day';\n";
            $delsql.= "delete from ac_srcaddr_sid WHERE ip_src=$ip_src and day='$day' and sid=$sid;\n";
            $delsql.= "delete from ac_srcaddr_signature WHERE ip_src=$ip_src and day='$day' and signature=$signature;\n";
            $delsql.= "delete from ac_srcaddr_ipdst WHERE ip_src=$ip_src and day='$day' and ip_dst=$ip_dst;\n";
            // AC_DSTADDR
            $delsql.= "update ignore ac_dstaddr_ipdst set cid=cid-1 WHERE ip_dst=$ip_dst and day='$day';\n";
            $delsql.= "delete from ac_dstaddr_sid WHERE ip_dst=$ip_dst and day='$day' and sid=$sid;\n";
            $delsql.= "delete from ac_dstaddr_signature WHERE ip_dst=$ip_dst and day='$day' and signature=$signature;\n";
            $delsql.= "delete from ac_dstaddr_ipsrc WHERE ip_dst=$ip_dst and day='$day' and ip_src=$ip_src;\n";
            // AC_LAYER4_SRC
            $delsql.= "update ignore ac_layer4_sport set cid=cid-1 WHERE layer4_sport=$layer4_sport and ip_proto=$ip_proto and day='$day';\n";
            $delsql.= "delete from ac_layer4_sport_sid WHERE layer4_sport=$layer4_sport and ip_proto=$ip_proto and day='$day' and sid=$sid;\n";
            $delsql.= "delete from ac_layer4_sport_signature WHERE layer4_sport=$layer4_sport and ip_proto=$ip_proto and day='$day' and signature=$signature;\n";
            ...
            // AC_LAYER4_DST
            $delsql.= "update ignore ac_layer4_dport set cid=cid-1 WHERE layer4_dport=$layer4_dport and ip_proto=$ip_proto and day='$day';\n";
            $delsql.= "delete from ac_layer4_dport_sid WHERE layer4_dport=$layer4_dport and ip_proto=$ip_proto and day='$day' and sid=$sid;\n";
            $delsql.= "delete from ac_layer4_dport_signature WHERE layer4_dport=$layer4_dport and ip_proto=$ip_proto and day='$day' and signature=$signature;\n";
