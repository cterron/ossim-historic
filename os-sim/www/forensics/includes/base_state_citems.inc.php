...
class MultipleElementCriteria extends BaseCriteria {
    ...
    function PrintForm($field_list, $blank_field_string, $add_button_string) {
        for ($i = 0; $i < $this->criteria_cnt; $i++) {
            ...
            echo '    </SELECT>';
            if ($i == $this->criteria_cnt - 1) echo '    <INPUT TYPE="submit" class="button" NAME="submit" VALUE="' . $add_button_string . '">';
            echo '<BR>';
        }
    }
    function Compact() {
    ...
    */
    ...
    function Description() {
        ...
        if ((isset($this->criteria[0])) && ($this->criteria[0] != " ") && (isset($this->criteria[1])) && ($this->criteria[1] != "")) {
            ...
            if (($this->db->baseGetDBversion() >= 100) && $this->sig_type == 1) $tmp = $tmp . BuildSigByID($this->criteria[1], $this->db) . '" ' . $this->cs->GetClearCriteriaString($this->export_name);
    ...
    */
    ...
    function PrintForm() {
        for ($i = 0; $i < $this->criteria_cnt; $i++) {
            ...
            if ($GLOBALS['ip_address_input'] == 2) echo '    <INPUT TYPE="text" NAME="ip_addr[' . $i . '][3]" SIZE=16 VALUE="' . @$this->criteria[$i][7] . '">';
            else {
    ...
    function Description() {
        ...
        for ($i = 0; $i < $this->criteria_cnt; $i++) {
            ...
            if ($tmp != "") {
                ...
                $tmp2 = $tmp2 . $this->criteria[$i][0] . $human_fields[($this->criteria[$i][1]) ] . ' ' . $this->criteria[$i][2] . ' ' . $tmp . ' ' . $this->criteria[$i][8] . ' ' . $this->criteria[$i][9] . $mask . $this->cs->GetClearCriteriaString($this->export_name) . "<BR>";
            }
        }
        return $tmp2;
    }
    ...
    */
    ...
    function Description() {
        ...
        if (isset($this->criteria[0]) && ($this->criteria[0] != " ") && ($this->criteria[0] != "")) {
            ...
            for ($i = 8; $i >= 1; $i--) if ($this->criteria[$i] == "") $tmp = $tmp . '-';
            else $tmp = $tmp . $human_fields[($this->criteria[$i]) ];
