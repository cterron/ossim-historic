    ...
    for ($i = 0; $i < $cnt; $i++) {
        ...
        if (isset($field[$i]) && $field[$i][1] != " " && $field[$i][1] != "") {
                    ...
                    if ($db->DB_type == "oci8") {
                        ...
                        if (count($field) > 1) {
                            ...
                            continue;
                                ...
                                if ($i < $cnt - 1) $tmp = $field[$i][0] . " timestamp " . $op . "'$t'" . $field[$i][8] . ' ' . $field[$i][9];
            ...
            /* Build the SQL string when the = operator is used */
                ...
                else if ($i < $cnt - 1) $tmp = $field[$i][0] . $tmp . ')' . $field[$i][8] . $field[$i][9];
...
function DataRows2sql($field, $cnt, $data_encode, &$s_sql) {
    ...
    for ($i = 0; $i < $cnt; $i++) {
        ...
        if ($field[$i][2] != "" && $field[$i][1] != " ") {
            ...
            $tmp = " acid_event.sid=data.sid AND acid_event.cid=data.cid AND (MATCH(data_payload) AGAINST ('" . FormatPayload($field[$i][2], $data_encode) . "' IN BOOLEAN MODE) OR data_payload LIKE '%" . FormatPayload($field[$i][2], $data_encode1) . "%')";
            //$tmp = " acid_event.sid=data.sid AND acid_event.cid=data.cid AND data_payload LIKE '%".FormatPayload($field[$i][2], $data_encode)."%'";
...
<TABLE BORDER=0 CELLSPACING=0 CELLPADDING=0 WIDTH="100%">
	<TR>
		<TD style="border:1px solid #ABB7C7">
			<TABLE BORDER=0 CELLSPACING=0 CELLPADDING=2 WIDTH="100%">
				<TR><TD class="sectiontitle">&nbsp;Current Criteria&nbsp;&nbsp; [<a href="base_qry_main.php?new=1&time_range=all&clear_allcriteria=1&submit=Query+DB" style="font-weight:normal">...Clear All Criteria...</a>]</TD></TR>
				<TR>
					<TD>
						<table border=0 WIDTH="100%">
							<tr>
								<th bgcolor="#D1D7DF">Meta</th>
								<th bgcolor="#D1D7DF">Payload</th>
								<th bgcolor="#D1D7DF" width="60">IP</th>
								<th bgcolor="#D1D7DF" nowrap>Layer 4</th>
							</tr>
							<tr>
								<td align=center valign="top"><?php echo $criteria_arr['meta'] ?></td>
								<td align=center valign="top"><?php echo $criteria_arr['payload'] ?></td>
								<td align=center valign="top"><?php echo $criteria_arr['ip'] ?></td>
...
function QuerySignature($q, $cmd) {
    ...
    $ids = "";
    $op = ($cmd == "=") ? "= '$q'" : "LIKE '%" . $q . "%'";
    $sql = "SELECT sig_id FROM signature WHERE sig_name $op";
    //echo $sql."<br>\n";
...
function ProcessCriteria() {
    ...
    if ((isset($sig[0]) && $sig[0] != " " && $sig[0] != "") && (isset($sig[1]) && $sig[1] != "")) {
        if (is_numeric($sig[1])) {
            $sig_neg = "";
            if ($sig[2] == "!=") $sig_neg = " NOT ";
            if ($db->baseGetDBversion() >= 100) {
                if (($sig_type == 1) && ($sig[0] == "=")) {
                    $tmp_meta = $tmp_meta . " AND (signature='" . $sig[1] . "') ";
                } else {
                    if ($sig[0] == "=") if ($db->DB_type != "mssql") $tmp_meta = $tmp_meta . " AND " . $sig_neg . " (sig_name='" . $sig[1] . "') ";
                    else $tmp_meta = $tmp_meta . " AND " . $sig_neg . " (sig_name LIKE '" . MssqlKludgeValue($sig[1]) . "') ";
                    else if ($sig[0] == "LIKE") $tmp_meta = $tmp_meta . " AND " . $sig_neg . " (sig_name LIKE '%" . $sig[1] . "%') ";
                }
            } else {
                if ($sig[0] == "=") $tmp_meta = $tmp_meta . " AND " . $sig_neg . " (signature='" . $sig[1] . "') ";
                else if ($sig[0] == "LIKE") $tmp_meta = $tmp_meta . " AND " . $sig_neg . " (signature LIKE '%" . $sig[1] . "%') ";
            }
        } else {
            // DK 2009/02/21: try query expansion for single words, could be interesting
            /*
            if(count(split(" ", $sig[1])) > 1){
            */
            // for acid_event MyISAM and "name" with FULLTEXT index, may be
            //$tmp_meta = $tmp_meta." AND MATCH(sig_name) AGAINST ('".$sig[1]."' IN BOOLEAN MODE) ";
            // for acid_event InnoDB first query throught signature table
            $sig_ids = QuerySignature($sig[1], $sig[0]);
            $sig_cmd = ($sig[2] == "!=") ? "not in" : "in";
            if ($sig_ids != "") $tmp_meta = $tmp_meta . " AND signature $sig_cmd ($sig_ids) ";
            /*
            } else {
            $tmp_meta = $tmp_meta." AND MATCH(sig_name) AGAINST ('".$sig[1]."' WITH QUERY EXPANSION) ";
            }
            */
        }
    ...
    */
    ...
    if ($ossim_risk_a != " " && $ossim_risk_a != "" && $ossim_risk_a != "0") {
        if ($ossim_risk_a == "low") {
            ...
            $tmp_meta = $tmp_meta . " AND ossim_risk_a < 1 ";
