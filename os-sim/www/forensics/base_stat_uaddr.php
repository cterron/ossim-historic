...
$sql = "SELECT DISTINCT $addr_type_name, " . " COUNT(acid_event.cid) as num_events," . " COUNT( DISTINCT acid_event.sid) as num_sensors, " . " COUNT( DISTINCT signature ) as num_sig, ";
if ($addr_type == DEST_IP) $sql = $sql . " COUNT( DISTINCT ip_src ) as num_sip ";
else $sql = $sql . " COUNT( DISTINCT ip_dst ) as num_dip ";
$sql = $sql . $sort_sql[0] . $from . $where . " GROUP BY $addr_type_name " . $sort_sql[1];
// use accumulate tables only with timestamp criteria
if ($use_ac) {
    ...
    if ($addr_type_name == "ip_src") {
        ...
        $sql = "SELECT DISTINCT ip_src,
			...
			(select count(distinct(signature)) from ac_srcaddr_signature where ac_srcaddr_ipsrc.ip_src=ac_srcaddr_signature.ip_src $sqlb) as num_sig,
			(select count(distinct(ip_dst)) from ac_srcaddr_ipdst where ac_srcaddr_ipsrc.ip_src=ac_srcaddr_ipdst.ip_src $sqlc) as num_dip
			FROM ac_srcaddr_ipsrc $where GROUP BY ip_src $orderby";
    } else {
        if (preg_match("/timestamp/", $criteria_clauses[1])) {
            $where = "WHERE " . str_replace("timestamp", "day", $criteria_clauses[1]);
        ...
        $sql = "SELECT DISTINCT ip_dst,
			...
			(select count(distinct(signature)) from ac_dstaddr_signature where ac_dstaddr_ipdst.ip_dst=ac_dstaddr_signature.ip_dst $sqlb) as num_sig,
			(select count(distinct(ip_src)) from ac_dstaddr_ipsrc where ac_dstaddr_ipdst.ip_dst=ac_dstaddr_ipsrc.ip_dst $sqlc) as num_dip
			FROM ac_dstaddr_ipdst $where GROUP BY ip_dst $orderby";
    }
}
//echo $sql;
...
while (($myrow = $result->baseFetchRow()) && ($i < $qs->GetDisplayRowCnt())) {
    ...
    */
    if ($no_ip) qroPrintEntry('<A HREF="' . $BASE_urlpath . '/help/base_app_faq.php#1">' . _UNKNOWN . '</A>');
        ...
        qroPrintEntry('&nbsp;&nbsp;' . BuildAddressLink($currentIP, 32) . $currentIP . '</A>&nbsp;' . $country_img);
    }
    if ($resolve_IP == 1) qroPrintEntry('&nbsp;&nbsp;' . baseGetHostByAddr($currentIP, $db, $dns_cache_lifetime) . '&nbsp;&nbsp;');
    /* Print # of Occurances */
    $tmp_iplookup = 'base_qry_main.php?new=1' . '&amp;num_result_rows=-1' . '&amp;submit=' . _QUERYDBP . '&amp;current_view=-1&amp;ip_addr_cnt=1';
    $tmp_iplookup2 = 'base_stat_alerts.php?new=1' . '&amp;num_result_rows=-1' . '&amp;submit=' . _QUERYDBP . '&amp;current_view=-1&amp;ip_addr_cnt=1';
    if ($addr_type == 1) {
