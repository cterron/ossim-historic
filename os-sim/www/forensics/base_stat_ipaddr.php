...
function PrintEventsByIP($db, $ip) {
    ...
    for ($i = 0; $i < $unique_event_cnt; $i++) {
        ...
        $tmp_iplookup = 'base_qry_main.php?new=1' . '&amp;sig%5B0%5D=%3D&amp;sig%5B1%5D=' . (rawurlencode(GetSignatureName($unique_events[$i], $db))) . '&amp;num_result_rows=-1' . '&amp;submit=' . _QUERYDBP . '&amp;current_view=-1&amp;ip_addr_cnt=2' . BuildIPFormVars($ip);
        $tmp_sensor_lookup = 'base_stat_sensor.php?' . 'sig%5B0%5D=%3D&amp;sig%5B1%5D=' . (rawurlencode($unique_events[$i])) . '&amp;ip_addr_cnt=2' . BuildIPFormVars($ip);
...
echo '<FONT>' . _PSEXTERNAL . ': ' . '<A HREF="' . $external_dns_link . $ip . '" target="_NEW">DNS</A> | ' . '<A HREF="' . $external_whois_link . $ip . '" target="_NEW">whois</A> | ' . '<A HREF="' . $external_all_link . $ip . '" target="_NEW">Extended whois</A> | ' . '<A HREF="http://www.dshield.org/ipinfo.php?ip=' . $ip . '&amp;Submit=Submit" target="_NEW">DShield.org IP Info</A> | ' . '<A HREF="http://www.trustedsource.org/query.php?q=' . $ip . '" target="_NEW">TrustedSource.org IP Info</A> | ' . '<A HREF="http://www.openrbl.org/?i=' . $ip . '" target="_NEW">Openrbl.org IP Info</A> | ' . '<A HREF="http://www.spamhaus.org/query/bl?ip=' . $ip . '" target="_NEW">Spamhaus.org IP Info</A> | ' . '<A HREF="http://www.spamcop.net/w3m?action=checkblock&ip=' . $ip . '" target="_NEW">Spamcop.net IP Info</A> | ' . '<A HREF="http://www.senderbase.org/senderbase_queries/detailip?search_string=' . $ip . '" target="_NEW">Senderbase.org IP Info</A> | ' . '<A HREF="http://isc.sans.org/ipinfo.html?ip=' . $ip . '" target="_NEW">ISC Source/Subnet Report</A><BR> </FONT>';
?>
</CENTER>
<HR>
...
if ($num_src_ip == 0) echo '<TD ALIGN="center">' . $num_src_ip;
else echo '<TD ALIGN="center"><A HREF="' . $tmp_src_iplookup . '">' . $num_src_ip . '</A>';
if ($num_dst_ip == 0) echo '<TD ALIGN="center">' . $num_dst_ip;
else echo '<TD ALIGN="center"><A HREF="' . $tmp_dst_iplookup . '">' . $num_dst_ip . '</A>';
echo '
