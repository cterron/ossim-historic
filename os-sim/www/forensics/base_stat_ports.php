...
$db->baseDBConnect($db_port_connect_method, $alert_dbname, $alert_host, $alert_port, $alert_user, $alert_password);
...
$qro->AddTitle(_UNIALERTS, "alerts_a", " ", " ORDER BY num_sig ASC", "alerts_d", " ", " ORDER BY num_sig DESC");
...
if ($use_ac) {
    ...
    if ($port_type_sql == "layer4_sport") {
        ...
        $sql = "SELECT DISTINCT layer4_sport, MIN(ip_proto),
			...
			(select count(distinct(signature)) from ac_layer4_sport_signature where ac_layer4_sport.layer4_sport=ac_layer4_sport_signature.layer4_sport $sqlb) as num_sig,
			...
			FROM ac_layer4_sport $where GROUP BY layer4_sport $orderby";
    } else {
        if (preg_match("/timestamp/", $criteria_clauses[1])) {
            $where = "WHERE " . str_replace("acid_event.", "", str_replace("timestamp", "day", $criteria_clauses[1]));
        ...
        $orderby = str_replace("acid_event.", "", $sort_sql[1]);
        $sql = "SELECT DISTINCT layer4_dport, MIN(ip_proto),
			sum(cid) as num_events,
			...
			(select count(distinct(signature)) from ac_layer4_dport_signature where ac_layer4_dport.layer4_dport=ac_layer4_dport_signature.layer4_dport $sqlb) as num_sig,
			...
			min(first_timestamp) as first_timestamp,  max(last_timestamp) as last_timestamp
			FROM ac_layer4_dport $where GROUP BY layer4_dport $orderby";
    }
}
//echo $sql;
...
while (($myrow = $result->baseFetchRow()) && ($i < $qs->GetDisplayRowCnt())) {
    ...
    $url_param = $url_port_type . "_port%5B0%5D%5B0%5D=+" . "&amp;" . $url_port_type . "_port%5B0%5D%5B1%5D=" . $port_type_sql . "&amp;" . $url_port_type . "_port%5B0%5D%5B2%5D=%3D" . "&amp;" . $url_port_type . "_port%5B0%5D%5B3%5D=" . $url_port . "&amp;" . $url_port_type . "_port%5B0%5D%5B4%5D=+" . "&amp;" . $url_port_type . "_port%5B0%5D%5B5%5D=+" . "&amp;" . $url_port_type . "_port_cnt=1" . "&amp;layer4=" . $url_layer4 . "&amp;num_result_rows=-1&amp;current_view=-1";
    ...
    qroPrintEntry('<A HREF="base_stat_alerts.php?' . $url_param . '&amp;sort_order=sig_a">' . $num_sig . '</A>');
