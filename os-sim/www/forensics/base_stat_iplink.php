...
while (($myrow = $result->baseFetchRow()) && ($i < $qs->GetDisplayRowCnt())) {
    ...
    if ($sip && $dip) {
        ...
        $s_country = strtolower(geoip_country_code_by_addr($gi, baseLong2IP($sip)));
        $s_country_name = geoip_country_name_by_addr($gi, baseLong2IP($sip));
        if ($s_country) {
            $s_country_img = " <img src=\"/ossim/pixmaps/flags/" . $s_country . ".png\" title=\"" . $s_country_name . "\">";
        } else {
            $s_country_img = "";
        }
        $d_country = strtolower(geoip_country_code_by_addr($gi, baseLong2IP($dip)));
        $d_country_name = geoip_country_name_by_addr($gi, baseLong2IP($dip));
        if ($d_country) {
        ...
        qroPrintEntry(BuildAddressLink(baseLong2IP($sip) , 32) . baseLong2IP($sip) . '</A>' . $s_country_img);
        qroPrintEntry('-->');
        qroPrintEntry(BuildAddressLink(baseLong2IP($dip) , 32) . baseLong2IP($dip) . '</A>' . $d_country_img);
