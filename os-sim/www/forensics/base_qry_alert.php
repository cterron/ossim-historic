...
function PrintPacketLookupBrowseButtons2($seq, $order_by_tmp, $where_tmp, $db, &$previous_button, &$next_button) {
    ...
    $where = $where_tmp;
    if ($seq < 1) {
        $sql = "SELECT acid_event.sid, acid_event.cid from acid_event $where $order_by limit $seq,2";
        //echo $sql;
        ...
        $sql = "SELECT acid_event.sid, acid_event.cid from acid_event $where $order_by limit " . intval($seq - 1) . ",3";
        //echo $sql;
        $result2 = $db->baseExecute($sql);
        $myrow2 = $result2->baseFetchRow();
...
if (preg_match("/data_payload/", $criteria_clauses[1])) {
    $where = ",data WHERE acid_event.sid = data.sid AND acid_event.cid=data.cid AND " . $criteria_clauses[1];
}
...
PrintPacketLookupBrowseButtons2($seq, $tmp_sql, $where, $db, $previous, $next);
              ...
              <TD>
                  <TABLE BORDER=0 CELLPADDING=4>
                    <TR><TD CLASS="plfieldhdr" >' . _ID . ' #</TD>
                        ...
                        <TD CLASS="plfieldhdr">' . _QATRIGGERSIG . '</TD>
                        <TD CLASS="plfieldhdr">' . _("Plugin Name") . '</TD>
                        <TD CLASS="plfieldhdr">' . _("Plugin ID") . '</TD>
                        <TD CLASS="plfieldhdr">' . _("Plugin SID") . '</TD></TR>
                    <TR><TD CLASS="plfield">' . ($sid . " - " . $cid) . '</TD>
                        <TD CLASS="plfield">' . $myrow2[1] . '</TD>
                        <TD CLASS="plfield">' . str_replace("[] ", "", GetTagTriger(BuildSigByID($myrow2[0], $db, 1, $plugin_id) , $db, $sid, $cid)) . '</TD>
                        <TD CLASS="plfield">' . $plugin_name . '</TD>
                        <TD CLASS="plfield">' . $plugin_id . '</TD>
                        <TD CLASS="plfield">' . $plugin_sid . '</TD></TR>
                  </TABLE>
              </TD>
           </TR>';
echo '  <TR>
...
if ($layer4_proto == "6") {
    ...
    echo '      <TD>';
    echo '         <TABLE BORDER=0 CELLPADDING=2>';
    echo '            <TR><TD class="plfieldhdr">' . _SHORTSOURCE . ' ' . _PORT . '</TD>
                            <TD class="plfieldhdr"> ' . _SHORTDEST . ' ' . _PORT . ' &nbsp</TD>';
    if (in_array($plugin_id, $snort_ids)) {
        echo '                <TD class="plfieldhdr">R<BR>1</TD>
...
if (in_array($plugin_id, $snort_ids)) {
    echo '<br><TABLE BORDER=0 cellpadding=2 cellspacing=0 class="bborder" WIDTH="90%">
           		<TR><TD class="payloadtitle" WIDTH=50 ROWSPAN=2 ALIGN=CENTER valign="top">Payload';
    ...
    echo '<br><TABLE BORDER=0 cellpadding=2 cellspacing=0 class="bborder" WIDTH="90%">
           <TR><TD class="payloadtitle" WIDTH=50 ROWSPAN=2 ALIGN=CENTER valign="top">Log';
}
echo '       <TD>';
if ($payload) {
    if (!in_array($plugin_id, $snort_ids)) {
        echo '      <TD>';
        echo '         <TABLE BORDER=0 CELLPADDING=2>';
        echo '            <TR><TD class="plfieldhdr">&nbsp;filename&nbsp;</TD>
                           <TD class="plfieldhdr">&nbsp;username&nbsp;</TD>
                           <TD class="plfieldhdr">&nbsp;password&nbsp;</TD>
                           <TD class="plfieldhdr">&nbsp;userdata1&nbsp;</TD>
                           <TD class="plfieldhdr">&nbsp;userdata2&nbsp;</TD>
                           <TD class="plfieldhdr">&nbsp;userdata3&nbsp;</TD>
                           <TD class="plfieldhdr">&nbsp;userdata4&nbsp;</TD>
                           <TD class="plfieldhdr">&nbsp;userdata5&nbsp;</TD>
                           <TD class="plfieldhdr">&nbsp;userdata6&nbsp;</TD>
                           <TD class="plfieldhdr">&nbsp;userdata7&nbsp;</TD>
                           <TD class="plfieldhdr">&nbsp;userdata8&nbsp;</TD>
                           <TD class="plfieldhdr">&nbsp;userdata9&nbsp;</TD>';
