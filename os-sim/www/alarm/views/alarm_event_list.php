...
<script>
	...
	{	
		$.ajax({
			...
			type: "POST",
			url: "<?php echo $alarm_url['view'] ?>alarm_events.php", 
			dataType: "html",
			beforeSend: function()
			{
			...
			{ 
                ...
                $('#event_detail').find('.otx_icon').off('click').on('click', function()
                {
                    var id      = $(this).data('event');
                    var e_alarm = "<?php echo $backlog_id ?>";
                    var title   = "<?php echo _('OTX DETAILS') ?>";
                    var url     = "/ossim/otx/views/view_my_pulses.php?type=alarm_event&id=" + id + "&alarm_event="+e_alarm;
