...
class AVSysConfig (object):
    ...
    def set_net_iface_config (self, iface, address = None, netmask = None, gateway = None, \
        ...
        if dns_search != None:
            self.__augeas.set(iface_path + '/dns-search', dns_search)
        ...
        if local_ip is not None and local_netmask is not None:
            ...
            if broadcast is None:
                self.__augeas.set(iface_path + '/broadcast', net_iface_broadcast)
                self.__pending['%s broadcast' % iface] = net_iface_broadcast
            if network is None:
                self.__augeas.set(iface_path + '/network', net_iface_network)
                self.__pending['%s network' % iface] = net_iface_network
