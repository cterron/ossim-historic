...
class JobResult():
    ...
    @property
    def serialize(self):
        return {'result': self.result, 'message': self.message, 'log_file': self.log_file, 'error_id': self.error_id}
...
def get_task_status(system_id, system_ip, task_list):
    ...
    for name, my_task in task_list.iteritems():
        ...
        if success:
            ...
            logger.warning("Cannot retrieve the process pid %s:%s" % (success, pid))
            return False, {}
