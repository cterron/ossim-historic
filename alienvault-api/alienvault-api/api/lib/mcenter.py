...
def verify_sign(signature, data):
    """Verfify that the given data comes from the expected source
    Args:
...
def get_message_center_messages():
    ...
    try:
        ...
        url = 'https://%s:%s/messages?%s' % (app.config['MESSAGE_CENTER_SERVER'],
        ...
        import traceback
        app.logger.error("An error occurred while retrieving the Message Center Server messages: %s" % str(traceback.format_exc()))
